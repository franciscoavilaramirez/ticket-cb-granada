...
export class ApiService {
  ...
  resetPassword(token: string, password: string) {
    return this.http.post(`${environment.apiUrl}password-reset/confirm`, { token, newPassword: password });
  }

  verifyResetToken(token: string) {
    return this.http.get(`${environment.apiUrl}password-reset/verify`, {
      params: { token }
    });
  }
